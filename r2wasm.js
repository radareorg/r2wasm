...
class RadareProgressElement extends HTMLElement {
  connectedCallback() {
    ...
    const htmlString = "<div><button onclick='r2wasm_init_async()'>r2wasm.init()</button></div>";
    const parser = new DOMParser();
    const doc = parser.parseFromString(htmlString, 'text/html');
    //this.shadowRoot.append(doc.body.firstChild);
    this.appendChild(doc.body.firstChild);
...
function r2wasm_progress(detail) {
  ...
  if (percent === 100) {
    ...
    pc.innerHTML = "<button onclick='r2wasm_cancel()'>cancel</button> <div style='border:2px solid black;background-color:grey;width:"+percent+"px'> r2wasm("+percent+"%)</div>";
  }
}
